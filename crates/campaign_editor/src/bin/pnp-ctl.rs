...
//!
//! ```text
//! pnp-ctl <command> [args...]
//! pnp-ctl run <scenario file>
//! ```
//!
//! The socket comes from `PNP_CONTROL`, the same variable the editor was started with.
//! One command per connection: connect, write a line, read a line of JSON.
//!
//! **Blocking is the feature.** The editor holds each reply until the command has really
//! happened, so `click` returns once the frame carrying the press has run and `capture`
//! once the PNG is on disk. There is nothing to poll and nothing to sleep on.
//!
...
const SOCKET_ENV: &str = "PNP_CONTROL";

fn main() -> ExitCode {
    let args: Vec<String> = std::env::args().skip(1).collect();
    if args.is_empty() {
        eprintln!("usage: pnp-ctl <command> [args...]");
        eprintln!("       pnp-ctl run <scenario file>");
...
fn scenario(socket: &str, path: &str) -> ExitCode {
    ...
    for line in text.lines() {
        ...
        match send(socket, command) {
            ...
            Err(error) => {
                replies.push(entry(command, &format!("{{\"ok\":false,\"error\":{error:?}}}")));
                failed = true;
                break;
            }
        }
