//! The socket the editor is driven through: accepting a caller, reading its command, and
//! holding the reply until the command has finished happening.
//!
//! One client at a time, and one command per connection. Commands are a sequence, and a
//! second caller interleaving its own would make "what state is the editor in"
//! unanswerable; a caller that connects while a command is in flight waits in the accept
//! queue.
...
pub(super) const SOCKET_ENV: &str = "PNP_CONTROL";

/// Binds the socket named by `PNP_CONTROL` and installs the system that serves it.
///
/// Does nothing when the variable is unset, and logs rather than failing when the socket
/// cannot be bound — the editor runs either way, because a control socket is a way to
/// talk to the tool and never a thing the tool needs.
///
/// Serves in `PreUpdate` **after** the input systems, not before: those clear
/// `just_pressed` at the start of every frame, so a synthetic press written before them
/// would be wiped before any authoring system could see it.
pub(super) fn build(app: &mut App) {
    ...
    match ControlServer::bind(path.clone()) {
        Ok(server) => {
            ...
            app.add_systems(PreUpdate, serve.after(InputSystems));
        }
    ...
    fn accept(&mut self) {
        ...
        match self.listener.accept() {
            Ok((stream, _)) => match stream.set_nonblocking(true) {
                ...
                Err(error) => warn!("control client rejected: {error}"),
            },
...
impl Client {
    fn read_command(&mut self) -> std::io::Result<Option<String>> {
        ...
        let line = String::from_utf8_lossy(&self.input[..end]).trim().to_owned();
